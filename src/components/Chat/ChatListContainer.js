      ...
      roomList.map((room) => (
        <li key={room.id} className={listItemClass}>
          <Link to={`/chatroom/${room.id}`} className={listLinkClass}>
            ...
            {/* 새 채팅 여부 */}
            {room.newMessage && (
              <span className="absolute top-3 right-3 flex w-5 h-5">
                <span className="animate-bounce w-5 h-5 flex justify-center items-center text-sm font-bold text-white rounded-full bg-rose-700">
                  N
                </span>
                <span className="sr-only">새 채팅 있음</span>
              </span>
            )}
